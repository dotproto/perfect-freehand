  ...
  */
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    */
    ...
    const angle = Math.atan2(vector[1] - nextVector[1], vector[0] - nextVector[0]);

    const isPointSharpCorner = prevDpr < 0 && !isPrevPointSharpCorner
    const isNextPointSharpCorner = nextDpr !== null && nextDpr < 0

    console.log({angle, nextDpr, prevDpr, isPrevPointSharpCorner, isPointSharpCorner, isNextPointSharpCorner });
    ...
    if (isPointSharpCorner || isNextPointSharpCorner) {
      ...
      const lRot = FIXED_PI - angle/2;
      const rRot = FIXED_PI + angle/2;

      // const dir = nextCpr > 0 ? 1 : -1;
      // const lRot = FIXED_PI + ang();
      // const rRot = FIXED_PI * 1;

      const step = 1 / CORNER_CAP_SEGMENTS
      for (let t = 0; t <= 1; t += step) {
        // Calculate left point: rotate (point - offset) around point
        subInto(_tl, point, _offset)
        rotAroundInto(_tl, _tl, point, lRot * t)
        ...
        rotAroundInto(_tr, _tr, point, rRot * -t)
